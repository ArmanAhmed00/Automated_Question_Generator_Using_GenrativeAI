...
# Configure Gemini API using API key from config
...
def test_generate():
    if request.method == "POST":
        ...
        if testType == "subjective":
            try:
                ...
                logging.debug(
                    f"Sending request to Gemini: Generate {noOfQues} questions based on the text.")
                response = chat.send_message(
                    f"Generate {noOfQues} questions and answers based on the following text. Format as 'Question: <question_text>', 'Answer: <answer_text>':\n{inputText}")
        ...
        # Use Gemini API for objective question generation
            ...
            try:
                ...
                response = chat.send_message(
                    ...
                    f"(D) <option4> as radio button, Answer: <correct_answer>:\n{inputText}"
                ...
                for line in lines:
                    ...
                    if line.startswith("Question:"):
                        if question_block:
                            structured_output.append("<br>".join(
                                question_block))  # Store previous block
        ...
        # Use existing logic for subjective question generation using local method
            ...
            try:
                response = generate_questions_with_ai(
                    inputText, noOfQues, "subjective")
                return render_template('generatedtestdata.html', cresults=response)
            except Exception as e:
                logging.error(
                    f"Error generating subjective questions: {str(e)}")
    ...
    # Default return for non-POST requests
    return redirect(url_for('index'))

# Function to generate questions based on existing local logic (objective/subjective)


def generate_questions_with_ai(input_text: str, num_questions: str, question_type: str) -> List[Tuple[str, str]]:
